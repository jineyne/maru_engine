...
struct rhi_device { int dummy; };
struct rhi_swapchain { int dummy; };
struct rhi_buffer { size_t size; };
struct rhi_texture { int w, h; };
struct rhi_shader { int linked; };
struct rhi_pipeline { rhi_shader_t* sh; };
struct rhi_cmd { int dummy; };
struct rhi_fence { int dummy; };
...
static void gles_destroy_device(rhi_device_t* d) { free(d); }
...
static rhi_cmd_t* gles_begin_cmd(rhi_device_t* d) {
    UNUSED(d); return (rhi_cmd_t*)calloc(1, sizeof(rhi_cmd_t));
}

static void gles_end_cmd(rhi_cmd_t* c) { free(c); }

static void gles_cmd_begin_render(rhi_cmd_t* c, rhi_texture_t* color, rhi_texture_t* depth, const float clear[4]) {
    UNUSED(c); UNUSED(color); UNUSED(depth); UNUSED(clear);
}

static void gles_cmd_end_render(rhi_cmd_t* c) { UNUSED(c); }
...
static void gles_cmd_draw(rhi_cmd_t* c, uint32_t vtx_count, uint32_t first, uint32_t inst) {
    UNUSED(c); UNUSED(vtx_count); UNUSED(first); UNUSED(inst);
}

static void gles_cmd_draw_indexed(rhi_cmd_t* c, uint32_t idx_count, uint32_t first, uint32_t base_vtx, uint32_t inst) {
    UNUSED(c); UNUSED(idx_count); UNUSED(first); UNUSED(base_vtx); UNUSED(inst);
}

/* ---- Sync ---- */
static rhi_fence_t* gles_fence_create(rhi_device_t* d) {
    UNUSED(d); return (rhi_fence_t*)calloc(1, sizeof(rhi_fence_t));
}
